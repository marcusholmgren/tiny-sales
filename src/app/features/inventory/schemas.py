...
class CategoryBase(BaseModel):
    name: str = Field(
        ..., min_length=1, max_length=100, description="Name of the category"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Optional description for the category"
    )
...
class CategoryUpdate(BaseModel):
    name: Optional[str] = Field(
        None, min_length=1, max_length=100, description="New name of the category"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="New description for the category"
    )


class CategoryResponse(CategoryBase):
    public_id: str = Field(
        ..., description="Public unique identifier for the category (KSUID)"
    )
...
class InventoryItemBase(BaseModel):
    name: str = Field(
        ..., min_length=1, max_length=255, description="Name of the inventory item"
    )
    quantity: int = Field(
...
class InventoryItemCreate(InventoryItemBase):
    category_id: Optional[str] = Field(
        None, description="Public ID of the category to assign to the item"
    )


class InventoryItemUpdate(BaseModel):
    name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="New name of the inventory item"
    )
...
class InventoryItemResponse(InventoryItemBase):
    public_id: str = Field(
        ..., description="Public unique identifier for the item (KSUID)"
    )
    category: Optional[CategoryResponse] = Field(
        None, description="Category of the inventory item"
    )
    created_at: datetime.datetime = Field(
