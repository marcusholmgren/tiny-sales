...
class Order(TimestampMixin):
    ...
    order_id = fields.CharField(max_length=50, unique=True, description="Pattern: <year+0000> e.g. 20250001")
    ...
    user: fields.ForeignKeyRelation["app.features.auth.models.User"] = fields.ForeignKeyField(
        "app.features.auth.models.User", related_name="orders", on_delete=fields.SET_NULL, null=True
    )
...
class OrderItem(TimestampMixin):
    ...
    item: fields.ForeignKeyRelation["app.features.inventory.models.InventoryItem"] = fields.ForeignKeyField(
        "app.features.inventory.models.InventoryItem", related_name="order_items_relation", on_delete=fields.RESTRICT
    )
...
class OrderEvent(models.Model): # No TimestampMixin
    ...
    order: fields.ForeignKeyRelation[Order] = fields.ForeignKeyField(
        "models.Order", related_name="events", on_delete=fields.CASCADE # Refers to local Order model
    )
    ...
    def __str__(self):
        order_id_val = self.order.order_id if hasattr(self.order, 'order_id') and self.order.order_id else 'N/A'
        return f"Event '{self.event_type}' for Order {order_id_val} at {self.occurred_at}"
